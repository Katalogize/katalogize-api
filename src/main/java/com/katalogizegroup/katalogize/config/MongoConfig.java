...
public class MongoConfig {
    ...
    CommandLineRunner commandLineRunner(CatalogRepository catalogRepository, CatalogItemRepository catalogItemRepository, UserRepository userRepository, CatalogTemplateRepository templateRepository) {
        if (catalogRepository.findAll().size() == 0 && userRepository.findAll().size() == 0) {
            return strings -> {
                userRepository.insert(new User((int)sequenceGenerator.generateSequence(User.SEQUENCE_NAME), "User1", "Mock1", "katalogize@email.com", false, "FakeUser1", null));
                userRepository.insert(new User((int)sequenceGenerator.generateSequence(User.SEQUENCE_NAME), "User2", "Mock2", "katalogize2@email.com", false, "FakeUser2", null));
